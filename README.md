# custom-redis
Lafaso_redis提供了多种访问redis的方式，具体使用方式参考如下:
...
依赖配置文件:
Classpath:/ redisrouting.xml

使用代码:
/*
cloudyCart指应用名,具体解释参考redisrouting.xml解释部分
ILafasoRedis是jedis的封装,通过ILafasoRedis可以执行redis的所有方法
*/
ILafasoRedis redis = factory.getLafasoRedisByAppId("cloudyCart", token);
redis.get("redisIP");


配置文件redisrouting.xml说明:
Lafaso_redis按照应用为单位管理redis，可以在一个redisrouting.xml文件中配置多个应用，每个应用对应了多种方式实现redis集群的管理。
