...
public class RedisClusterNodeMonitor implements Runnable {
	...
	private final static Logger log = Logger.getLogger(RedisClusterNodeMonitor.class);
	...
	public void run() {
		...
		while (alive) {
			try {
				...
				e.printStackTrace();
				log.error("LafasoRedis error forceDetectingBadNode forceDetectingNode exception cause:"+e.getMessage());
